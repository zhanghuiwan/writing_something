d:\BYDProject\qwen2vl-flux\model_origin.pyimport torch
...
def get_model_path(model_name):
    ...
    base_dir = os.getenv('CHECKPOINT_DIR', 'model_cache')  # Allow environment variable override
...
class FluxModel:
    ...
    def print_system_info(self):
        ...
        total_cpu_mem = virtual_mem.total / (1024 ** 3)  # 转换为 GB
        used_cpu_mem = virtual_mem.used / (1024 ** 3)  # 转换为 GB
        free_cpu_mem = virtual_mem.available / (1024 ** 3)  # 转换为 GB
        
        print(f"CPU Usage: {cpu_usage:.2f}%")
        print(f"Total Memory: {total_cpu_mem:.2f} GB")
        print(f"Used Memory: {used_cpu_mem:.2f} GB")
        print(f"Free Memory: {free_cpu_mem:.2f} GB\n")
        
        if torch.cuda.is_available():
            print("=== GPU Info ===")
            device = torch.cuda.current_device()
            gpu_name = torch.cuda.get_device_name(device)
            total_gpu_mem = torch.cuda.get_device_properties(device).total_memory / (1024 ** 3)  # 转换为 GB
            allocated_gpu_mem = torch.cuda.memory_allocated(device) / (1024 ** 3)  # 转换为 GB
            reserved_gpu_mem = torch.cuda.memory_reserved(device) / (1024 ** 3)  # 转换为 GB
            free_gpu_mem = reserved_gpu_mem - allocated_gpu_mem  # 剩余显存

            print(f"GPU Name: {gpu_name}")
            print(f"Total GPU Memory: {total_gpu_mem:.2f} GB")
            print(f"Allocated GPU Memory: {allocated_gpu_mem:.2f} GB")
            print(f"Reserved GPU Memory: {reserved_gpu_mem:.2f} GB")
            print(f"Free GPU Memory: {free_gpu_mem:.2f} GB\n")
        else:
    ...
    def _init_base_models(self):
        ...
        self.qwen2vl.requires_grad_(False).to("cpu")
        ...
        self.vae.requires_grad_(False).to(self.dtype).to("cpu")
        print("vae 已加载")
        self.print_system_info()
        self.transformer.requires_grad_(False).to(self.dtype).to("cpu")
        print("transformer 已加载")
        self.print_system_info()
    ...
    def process_image(self, image):
        ...
        with torch.no_grad():
            ...
            print("qwen2vl to cuda")
            self.qwen2vl.to("cuda")
            ...
            image_hidden_state = image_hidden_state.to("cpu")
            image_grid_thw = image_grid_thw.to("cpu")
            self.qwen2vl.to("cpu")
            torch.cuda.empty_cache()
            print("qwen2vl to cpu")
            self.print_system_info()
    ...
    def generate(self, input_image_a, input_image_b=None, prompt="", guidance_scale=3.5, num_inference_steps=1,
        ...
        # IMAGE GENERATION
        #############################
        self.vae.requires_grad_(False).to(self.dtype).to(self.devie)
        print("vae 已加载到 cuda")
        self.transformer.requires_grad_(False).to(self.dtype).to(self.device)
        print("transformer 已加载到 cuda")
        if mode == "variation":
